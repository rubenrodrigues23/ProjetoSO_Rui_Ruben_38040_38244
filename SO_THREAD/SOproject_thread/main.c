...
int main(int argc, char *argv[]) {
    ...
    read_genomes(gl, "/Users/andrebarbosa/CLionProjects/SOproject_thread/input/cds.fna");
    ...
    long i;
    int j;
    n_worker_threads = atoi(argv[1]);
    pthread_t thread[n_worker_threads];
    ...
    for (i = 0; i < n_worker_threads; i++) {
        ...
        pthread_create(&thread[i], NULL, &workerThread, (void *)i);
    }
        pthread_create(&thread_state, NULL, &printState, (void *)i );
...
void *workerThread(void *param){
    ...
    while (g != NULL) {
        ...
        save_mutation_array(mutation_array, "/Users/andrebarbosa/CLionProjects/SOproject_thread/result/thread_mutations.txt", 0, pthread_self());
        free_mutations(mutation_array);
        for (int k = 0; k < n_worker_threads; k++) {    //salto de cada filho
            if (g == NULL)
                pthread_exit(NULL);
            g = g->pnext;
        }
...
void *printState(void* param){
    ...
    while (1){
        pthread_mutex_lock(&mutex);
        int percentage = (int) (((float)n_comparacoes / ngenomes) * 100);
        pthread_mutex_unlock(&mutex);
